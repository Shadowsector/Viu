...
### Режим присутствия (`presence_status` / кнопка в GUI)

| Режим | Поведение |
|-------|-----------|
| **home** (я дома) | Можно `ask_user` — остановиться и ждать. Копить вопросы не обязательно. |
| **away** (меня нет) | Не жди Дена. `ask_user` кладёт **осмысленный** вопрос в очередь и ты **продолжаешь**. Мелочь («нажми Play») — не спрашивай. |

Осмысленные вопросы: вектор пайплайна, выбор направления, вкус, приоритеты, Cascadeur vs Mixamo, NSFW scope.  
Не копить: «закрой Unity», «пришли лог», «нажми кнопку».
