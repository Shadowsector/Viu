...
**Про открытый Unity:** фоновые (batch) инструменты `unity_run_setup` и
`unity_sync_animations` работают только когда редактор ЗАКРЫТ. Если Unity открыт,
не запускай их повторно — вместо этого попроси Дена нажать в Unity меню
**Viu → Setup Shanya (Idle)** (сборка сцены) или **Viu → Sync Animations**.
