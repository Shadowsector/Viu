...
BODY_STEPS: Tuple[Dict[str, str], ...] = (
    {
        "id": "stage_pack",
        "title": "1. Положить тела в Inbox",
        "how": (
            "Нужны ДВА файла/пака из хламника U:\\Desktop Mascot\\Women "
            "(не из установки HS2 на D:):\n"
            "\n"
            "A) Образец формы — Erisa (Smutbase / RedEyes). Только пропорции.\n"
            "B) Рабочее тело — другое тело из того же хламника "
            "(часто пак с Smutbase / DAZ-база с нормальной лицензией CC0/CC-BY). "
            "Именно оно потом пойдёт в игру.\n"
            "\n"
            "«Своё» = не твоё фото и не ваниль из HS2, а тело из ТВОЕЙ библиотеки "
            "Desktop Mascot, которое ты выбрал для Анабарры.\n"
            "\n"
            "Скопируй оба в U:\\Anabarra\\Inbox\\creatures\\ "
    ...
    },
    {
        ...
        "how": (
            "1) File → Open — открой .blend образца (Erisa) из Inbox.\n"
            "2) Положи рядом рабочее тело:\n"
            "   • если это .blend — File → Append → Object → выбери mesh тела;\n"
            "   • если .fbx / .obj — File → Import.\n"
            "3) В сцене должны быть видны ОБА меша: образец + рабочее.\n"
            "\n"
            "Откуда файл рабочего тела? Из той же папки Women в Desktop Mascot "
            "(или уже лежит в Inbox рядом с Erisa). Blender сам его не скачает — "
            "только откроет то, что ты положил на диск."
        ),
    },
    {
        "id": "shrinkwrap",
        ...
        "how": (
            "Цель: рабочее тело станет похоже на Erisa по фигуре, "
            "но останется «своим» мешем для игры.\n"
            "\n"
            "1) Выдели РАБОЧЕЕ тело (не Erisa).\n"
            "2) Modifiers → Add → Shrinkwrap → Target = Erisa "
            "(или как у тебя называется образец).\n"
            "3) Крути настройки, пока форма ок (как ты уже делал раньше).\n"
            "4) Когда доволен — Apply у модификатора (по желанию) и Ctrl+S.\n"
            "\n"
            "Erisa после этого можно спрятать или удалить из экспорта — "
            "в Unity уезжает рабочее тело."
        ),
    },
    {
        "id": "rigify",
        ...
        "how": (
            "Риг ставь на РАБОЧЕЕ тело (то, что подогнал), не на Erisa.\n"
            "Добавь Rigify → привяжи меш → сохрани .blend.\n"
