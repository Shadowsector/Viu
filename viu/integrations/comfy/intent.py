...
_COMFY_JOB_RE = re.compile(
    r"(?i)(?:"
    r"сгенер|генерир|сделай\s+видео|сними|снять|запусти|очеред|"
...
def format_reflect_comfy_block(config: Optional[Config] = None) -> str:
    ...
    lines = [
        ...
        "Reflect-чат сам jobs в очередь не ставит.",
    ]
    if paused:
        lines.append(
            "Сейчас Comfy на паузе (тело Шани / docs/NOW.md). "
            "Скажи это тепло; когда снимем паузу — Студия Comfy или Lab."
        )
    else:
        lines.append(
            "Направь: Студия Comfy, Lab, «покажи промпт» / Промпт Wan. "
            "Не обещай, что уже сняла, пока Ден не нажал."
        )
    return "\n".join(lines)
