...
class ViuGUI:
    def __init__(self) -> None:
        ...
        # story_memory: ingest логов + короткий чистый засев (без ассистентского яда).
        try:
            ...
            seeded = scrub_poisoned_history(
                get_story_memory(self.agent.config).as_chat_history(limit=6)
            )
            # Только хвост живого диалога — не 12 сырых битов.
            for turn in seeded[-6:]:
            ...
            if seeded:
                self._story_ingest_msg = (
                    ...
                    + f"в чат подхватила {len(seeded[-6:])} чистых реплик"
                )
        except OSError:
            self._story_ingest_msg = ""
    ...
    def _on_action(self, action: GuiAction) -> None:
        ...
        if action.tool == "__comfy_studio__":
            ...
            return
        if action.tool == "__comfy_shoot__":
            profile = str((action.tool_args or {}).get("profile") or "")
            style = str((action.tool_args or {}).get("style") or "realism")
        ...
        *,
        ...
        if profile in ("show", "шоу", "smoothmix", "beauty"):
            ...
            self._append(
                ...
                + "Снимаю по настройкам панели (промпт / длина / чекпоинт).",
                tag="viu",
            )
            start_args = {
                "topic": COMFY_TOPIC,
