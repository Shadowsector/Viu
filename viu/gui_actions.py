...
GUI_ACTIONS: List[GuiAction] = [
    ...
    GuiAction(
        ...
        hint="Пробная сцена: пустая площадка + Шаня + Idle↔Walk на A/D. "
        "Сейчас заточено под Шаню (не алхимичку и не волка — это позже). "
        "Вью сама закроет Unity, если он мешает, и откроет заново.",
    ),
    GuiAction(
        "unity_open",
        "Открыть Unity",
