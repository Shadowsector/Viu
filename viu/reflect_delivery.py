...
def list_delivery_hint(user_text: str) -> str:
    ...
    return (
        f"\n\n--- Несколько реплик ({n}) ---\n"
        f"Дену уйдут {n} коротких сообщений подряд. "
        f"Разбей так в ответе (final_parts). Формат вслух не обсуждай."
    )
...
def scene_delivery_hint(user_text: str) -> str:
    ...
    return (
        "\n\n--- Сцена ---\n"
        "Лучше две-три короткие реплики подряд: завязка, тело и действие, ощущение. "
        "От первого лица. Служебный формат Дена не касается."
    )
