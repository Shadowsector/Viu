"""Shoot intent: не стопорить Comfy на LoRA/промпте — TG только уведомляет."""
...
def test_shoot_intent_auto_picks_lora(tmp_path, monkeypatch):
    ...
    fake_spec = MagicMock(file="x.safetensors", strength=0.8)
...
def test_awaiting_prompt_with_shoot_continues(tmp_path, monkeypatch):
    """lab_start shoot=1 при awaiting_prompt — одобряет и продолжает, не стопорит."""
    cfg = _cfg(tmp_path, monkeypatch)
    session = new_session(COMFY_TOPIC)
