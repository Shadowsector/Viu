...
### Бег на четырёх (solo) — **в планах**

Отдельный режим локомоции: только когда Шаня одна, «кошачий» рывок на всех четырёх — **не** scope `creature_quadruped`, а `shanya_only` на том же Humanoid с enter/loop/exit.  
Черновик рига и пропорций: **`docs/notes/SHANYA_QUADRUPED_SPRINT_RIG.md`**.
