...
**В корне теперь один файл запуска — `Viu.cmd`.** Двойной клик по нему.
Он проверит обновления, установит пакет и откроет окно. Если что-то сломается,
окно останется с текстом ошибки (не исчезнет).

Все старые `start_viu.bat`, `get_viu.bat`, `check_unity.bat` и прочие переехали
в папку `legacy_scripts/` — трогать их не нужно. Ярлык на рабочий стол:
`setup/make_shortcut.bat`.
