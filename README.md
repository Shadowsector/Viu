...
1. Один раз запусти `make_shortcut.bat` — на рабочем столе появится ярлык
   **«Вью»** с яркой иконкой.
2. Дальше запускай Вью двойным кликом по этому ярлыку (или по `start_viu.bat`).

`start_viu.bat` по умолчанию подключает локальную **Ollama**
(`http://localhost:11434/v1`, модель `qwen2.5-coder:14b`) — параметры можно
поправить прямо в начале файла. Откроется графическое окно Вью.

Ещё два удобных файла (двойной клик):
- `console.bat` — открывает командную строку **сразу в папке проекта**
  (не нужно `cd`); можно набирать `python -m viu ...`.
- `check_blender.bat` — сразу проверяет связь с Blender (`blender_info` +
  `rig_check`) и показывает результат.
